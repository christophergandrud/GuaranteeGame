...
for omega in OmegaRange:
	for b in bRange:
		...
		nFloat = m.ceil(-0.5 + 0.5 * m.pow((omegaMax + 2/b), 0.5)) # Crawford & Sobel (1982, 1441)
		N = int(nFloat)
		print 'N: ', N

		# Find a1 cut point
		a1 = (omegaMax - 2 * N * (N - omegaMax) * b)/N # Crawford & Sobel (1982, 1441)
		...
		for i in range(2, Nless1 + 1):
			key = i
			a[key] = a1 * i + (2 * i) * (i - omegaMax) * b # Crawford & Sobel (1982, 1441)
			print 'a:', i, a[key]  
